		  				...
		  				<div class="card-header" style="height: 70px;">
		  					<div class="form-inline">
		  						...
		  						<p class="myspan-fraction">
									<span>共</span><samp>${exam.testpaper.squestionSize}</samp><span>题，
									<span>每题</span><samp>${exam.testpaper.squestionscore}</samp><span>分，
									合计</span><samp>${exam.testpaper.sTotal}</samp><span>分</span>
								</p>
		  					</div>
		  				</div>
	  				</c:if>
	    				...
	    				<c:forEach items="${exam.testpaper.squestions}" var="squestion" varStatus="status">
		    				<div id="sq-${status.index}" class="sqtimu">
		    					...
		    					<div class="radio radio-success radio-inline">
									<input type="radio" id="sq-${status.index}-option1" value="A" name="${status.index}">
									<label for="sq-${status.index}-option1">A. ${squestion.schoiceA }</label>
								</div>
								<div class="radio radio-success radio-inline">
									<input type="radio" id="sq-${status.index}-option2" value="B" name="${status.index}">
									<label for="sq-${status.index}-option2">B. ${squestion.schoiceB }</label>
								</div>
								<div class="radio radio-success radio-inline">
									<input type="radio" id="sq-${status.index}-option3" value="C" name="${status.index}">
									<label for="sq-${status.index}-option3">C. ${squestion.schoiceC }</label>
								</div>
								<div class="radio radio-success radio-inline">
									<input type="radio" id="sq-${status.index}-option4" value="D" name="${status.index}">
    				...
    				<c:if test="${not empty exam.testpaper.mquestions}">
		  				<div class="card-header" style="height: 70px;">
		  					<div class="form-inline">
		  						...
		  						<p class="myspan-fraction">
									<span>共</span><samp>${exam.testpaper.mquestionSize}</samp><span>题，
									<span>每题</span><samp>${exam.testpaper.mquestionscore}</samp><span>分，
									合计</span><samp>${exam.testpaper.mTotal}</samp><span>分</span>
								</p>
		  					</div>
		  				</div>
		  			</c:if>
	    				...
	    				<c:forEach items="${exam.testpaper.mquestions}" var="mquestion" varStatus="status">
		    				<div id="mq-${status.index}" class="mqtimu">
		    					...
		    					<div class="checkbox checkbox-success">
									<input id="mq-${status.index}-option1" class="styled" type="checkbox" value="A" name="${status.index}">
								    <label for="mq-${status.index}-option1">A. ${mquestion.mchoiceA}</label>
								</div>
								<div class="checkbox checkbox-success">
									<input id="mq-${status.index}-option2" class="styled" type="checkbox" value="B" name="${status.index}">
								    <label for="mq-${status.index}-option2">B. ${mquestion.mchoiceB}</label>
								</div>
								<div class="checkbox checkbox-success">
									<input id="mq-${status.index}-option3" class="styled" type="checkbox" value="C" name="${status.index}">
								    <label for="mq-${status.index}-option3">C. ${mquestion.mchoiceC}</label>
								</div>
								<div class="checkbox checkbox-success">
									<input id="mq-${status.index}-option4" class="styled" type="checkbox" value="D" name="${status.index}">
    				...
    				<c:if test="${not empty exam.testpaper.fquestions}">
		  				<div class="card-header" style="height: 70px;">
		  					<div class="form-inline">
		  						...
		  						<p class="myspan-fraction">
									<span>共</span><samp>${exam.testpaper.fquestionSize}</samp><span>题，
									<span>每题</span><samp>${exam.testpaper.fquestionscore}</samp><span>分，
									合计</span><samp>${exam.testpaper.fTotal}</samp><span>分</span>
								</p>
		  					</div>
		  				</div>
	  				</c:if>
	    				...
	    				<c:forEach items="${exam.testpaper.fquestions}" var="fquestion" varStatus="status">
		    				<div id="fq-${status.index}" class="mqtimu">
		    					...
		    					<div class="input-group input-group-sm">
								  ...
								  <input type="text" class="form-control myfullinput col-3" name="${status.index}" id= "fq-answer-${status.index}">
								</div>
		    				</div>
		    				<br />
	    				</c:forEach>
  				...
  				<div class="Answer-card">
	  				...
	  				<c:if test="${not empty exam.testpaper.squestions}">
	  					<div id="sq-card">
		  					...
		  					<div class="card-body" style="border-bottom: 1px solid #e4e4e4;">
		  						<c:forEach items="${exam.testpaper.squestions}" var="squestion" varStatus="status">
			  						<a href="#sq-${status.index}"><span class="myspan-Qu-Num" id="s-card-${status.index}">${status.index+1}</span></a>
		  						</c:forEach>
		  					</div>
	  					</div>
  					</c:if>
  					<c:if test="${not empty exam.testpaper.mquestions}">
	  					<div id="mq-card">
		  					...
		  					<div class="card-body" style="border-bottom: 1px solid #e4e4e4;" >
		  						<c:forEach items="${exam.testpaper.mquestions}" var="mquestion" varStatus="status">
		  							<a href="#mq-${status.index}"><span class="myspan-Qu-Num" id="m-card-${status.index}">${status.index+1}</span></a>
		  						</c:forEach>
		  					</div>
	  					</div>
  					</c:if>
  					<c:if test="${not empty exam.testpaper.fquestions}">
	  					<div id="fq-card">
							...
							<div class="card-body" style="border-bottom: 1px solid #e4e4e4;" >
								<c:forEach items="${exam.testpaper.mquestions}" var="mquestion" varStatus="status">
									<a href="#fq-${status.index}"><span class="myspan-Qu-Num" id="f-card-${status.index}">${status.index+1}</span></a>
								</c:forEach>
							</div>
						</div>
					</c:if>	
		...
		<script type="text/javascript">
			function submitExam(){
			    ...
			    /*$.post(url,param,function(data){
			    	// 提交成功
			    	if(data.msg == "yes"){
			        	window.location.href="${pageContext.request.contextPath }/exam_toEndExam?success=1";

			        	//提交失败
			        }else if(data.msg == "no"){
			        	window.location.href="${pageContext.request.contextPath }/exam_toEndExam?success=0";
			        }
			    },"json");*/
			}
		</script>
